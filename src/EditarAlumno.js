...
export default function EditarAlumno() {
    ...
    const fetchData = async () => {
      try {
        const [gradosRes, padresRes, alumnoRes] = await Promise.all([
          ...
          axios.get('http://localhost:4000/api/parents', { headers: { 'Authorization': `Bearer ${token}` } }),
          axios.get(`http://localhost:4000/api/students/${cui}`, { headers: { 'Authorization': `Bearer ${token}` } })
        ]);
  ...
  const onSubmit = async (e) => {
    ...
    const token = localStorage.getItem('accessToken');
    try {
      await axios.put(`http://localhost:4000/api/students/${cui}`, form, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
